...
const put = (url: string) => request(app).put(url).set('Authorization', `Bearer ${token}`)
const post = (url: string) => request(app).post(url).set('Authorization', `Bearer ${token}`)
const del = (url: string) => request(app).delete(url).set('Authorization', `Bearer ${token}`)

beforeAll(async () => {
  mongod = await MongoMemoryServer.create()
  await mongoose.connect(mongod.getUri('cotizador-test'))
  app = buildApp()
  token = (await request(app).post('/api/auth/login').send({ password: '1234' })).body.token
}, 180_000)
...
describe('perfil', () => {
  ...
  it('guarda y devuelve el perfil', async () => {
    ...
    await put('/api/profile').send(profile).expect(200)
    const res = await get('/api/profile')
...
describe('CRUD de cotizaciones', () => {
  it('PUT crea (upsert) y GET la devuelve sin campos internos de Mongo', async () => {
    await put('/api/quotes/q1').send(makeQuote('q1')).expect(200)
    const res = await get('/api/quotes/q1')
  ...
  it('DELETE elimina la cotización', async () => {
    await del('/api/quotes/q2').expect(200)
    await get('/api/quotes/q2').expect(404)
  })
})

describe('trazabilidad (revisions)', () => {
  it('registra una revisión create al crear', async () => {
    const res = await get('/api/quotes/q1/revisions')
    expect(res.status).toBe(200)
  ...
  it('registra revisión delete con el último snapshot', async () => {
    await del('/api/quotes/q1').expect(200)
    const res = await get('/api/quotes/q1/revisions')
...
describe('backup', () => {
  it('exporta todo e importa reemplazando', async () => {
    ...
    await del('/api/clients/c1').expect(200)
    expect((await get('/api/clients')).body.length).toBe(0)

    await post('/api/backup').send(backup.body).expect(200)
  ...
  it('rechaza un backup inválido', async () => {
    await post('/api/backup').send({ app: 'otra-cosa' }).expect(400)
  })
})
